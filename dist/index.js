    ...
    registerFull(api) {
        ...
        api.registerService({
            ...
            start(ctx) {
                ...
                if (!config.identity) {
                    ctx.logger.warn("Clawlink relay client not started: nostrPrivateKey is not configured.");
                    return;
                }
                ...
                service.startInbox({
                    onIncoming: async (event, decision) => {
                        ctx.logger.info(`[${CHANNEL_ID}] received ${event.envelope.type} from ${event.senderPublicKey}`);
                ...
                ctx.logger.info(`[${CHANNEL_ID}] relay client started for ${config.identity.npub}`);
            },
            stop(ctx) {
                service?.stop();
                service = undefined;
...
function formatEnvelopeForAgent(envelope, decision) {
