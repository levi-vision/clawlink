...
export const clawlinkChannelPlugin = createChatChannelPlugin<ClawlinkAccount>({
  base: {
    id: CHANNEL_ID,
    meta: {
      id: CHANNEL_ID,
    ...
    configSchema: channelConfigSchema as any,
    config: {
  ...
  outbound: {
    ...
    sendText: async (ctx) => {
      const service = new ClawlinkService({
        config: extractClawlinkConfigFromOpenClawConfig(ctx.cfg as OpenClawConfig),
      });
...
}) satisfies ChannelPlugin<ClawlinkAccount>;
