...
export function encodePairingInvitation(invitation: PairingInvitation): string {
  const payload = Buffer.from(JSON.stringify(invitation), "utf8").toString(
    "base64url",
  );
  return `clawlink1.${payload}`;
}

export function parsePairingInvitation(input: string): PairingInvitation {
  ...
  if (
    parsed?.protocol !== "clawlink.pairing" ||
    parsed?.version !== 1 ||
  ...
  return {
    ...
    publicKey,
    npub: encodePublicKey(publicKey),
    relays: parsed.relays.filter((relay: unknown): relay is string => {
...
export function assertPairingInvitationFresh(
